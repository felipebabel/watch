...
// Fetch all watched episodes for a show (season+episode numbers) from DB
async function fetchWatchedEpisodesForShow(userId: string, showDbId: string) {
  ...
  // Map "season-episode" -> episode_id
  const map = new Map<string, string>()
  data.forEach((r: any) => {
    const key = `${r.episodes.season}-${r.episodes.episode}`
    map.set(key, r.episode_id)
  })
  return map
}

// Upsert episode and toggle watched in one go
async function toggleEpisodeWatched(
  ...
  const { data: epData, error: epError } = await supabase
    .from('episodes')
    .upsert(
      { show_id: showDbId, season, episode, name },
...
export default function ShowPage() {
  ...
  // Map of "season-episode" -> episode_id for watched check
  const { data: watchedMap } = useQuery({
    queryKey: ['watched-map', user?.id, showDbId],
    queryFn: () => fetchWatchedEpisodesForShow(user!.id, showDbId),
  ...
  const toggleMutation = useMutation({
    mutationFn: async ({ season, episode, name, watched }: {
      ...
      // Auto-add as watching if not in list yet
  ...
  const handleAddShow = (status: WatchStatus) => {
    if (!show) return
    addShow.mutate({ tmdbId, name: show.name, posterPath: show.poster_path, status })
  }
  ...
  return (
    <div className="min-h-screen max-w-md mx-auto pb-8">
      ...
      <button
        onClick={() => navigate(-1)}
        className="fixed top-4 left-4 z-10 w-9 h-9 bg-black/60 backdrop-blur-sm rounded-full flex items-center justify-center text-white border border-white/10"
      >
        ←
      </button>
      ...
      <div className="px-4 -mt-16 relative">
        ...
        <div className="flex gap-4">
          <div className="w-24 h-36 rounded-xl overflow-hidden bg-surface-2 flex-shrink-0 border border-white/10 shadow-xl">
            {poster ? (
          ...
          <div className="flex flex-col justify-end gap-1 pt-20">
            ...
            <p className="text-xs text-muted">
              {show.first_air_date?.slice(0, 4)} · {show.number_of_seasons} temporada{show.number_of_seasons !== 1 ? 's' : ''}
            </p>
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          {userShow ? (
            <>
              <div className="flex-shrink-0 bg-accent/20 border border-accent/40 text-accent text-xs px-3 py-1.5 rounded-full">
                ✓ {STATUS_LABELS[userShow.status as WatchStatus]}
              </div>
              <button
                onClick={() => removeShow.mutate(showDbId)}
                className="flex-shrink-0 border border-white/10 text-muted text-xs px-3 py-1.5 rounded-full"
              >
                Remover
              </button>
            </>
            ...
            (['watching', 'watchlist', 'completed'] as WatchStatus[]).map((s) => (
              <button
                key={s}
                onClick={() => handleAddShow(s)}
                className="flex-shrink-0 border border-white/10 text-muted text-xs px-3 py-1.5 rounded-full hover:border-accent/50 hover:text-accent transition-colors"
              >
                + {STATUS_LABELS[s]}
              </button>
            ))
          ...
          {!seasonData ? (
            ...
            seasonData.episodes.map((ep) => {
              ...
              return (
                ...
                >
                  {/* Still */}
                  <div className="w-20 h-12 rounded-lg overflow-hidden bg-surface-2 flex-shrink-0">
                    {still ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {ep.runtime && (
                      <p className="text-[10px] text-muted mt-0.5">{ep.runtime} min</p>
                    )}
                  </div>
